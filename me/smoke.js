...
THREE.Ray.prototype = {
	...
	intersectObject: function(w) {
		var n, j, i, t, s, q, p, v, k, x, u, r, g = w.geometry,
			h = g.vertices,
			o, e = [],
			m;
...
THREE.Loader.prototype = {
	...
	loadBinary: function(a, e, b) {
		var c = (new Date).getTime(),
			d = new Worker(a);
		d.onmessage = function(h) {
			var g = h.data.materials,
				f = h.data.buffers;
			THREE.Loader.prototype.loadAjaxBuffers(f, g, e, b)
		};
		d.onerror = function(f) {
			alert("worker.onerror: " + f.message + "\n" + f.data);
			f.preventDefault()
		};
		d.postMessage(c)
	},
	loadAjaxBuffers: function(b, a, f, d) {
		var e = new XMLHttpRequest(),
			c = d + "/" + b;
		e.onreadystatechange = function() {
			if(e.readyState == 4) {
				if(e.status == 200 || e.status == 0) {
					THREE.Loader.prototype.createBinModel(e.responseText, f, d, a)
				} else {
					alert("Couldn't load [" + c + "] [" + e.status + "]")
				}
			}
		};
		e.open("GET", c, true);
	...
	createBinModel: function(c, e, b, a) {
		var d = function(k) {
				var g = this,
					z = 0,
					C, r = [];
				...
				function v(E, F) {
					var D = {
						signature: j(E, F, 8),
						header_bytes: x(E, F + 8),
						vertex_coordinate_bytes: x(E, F + 9),
						vertex_index_bytes: x(E, F + 10),
						normal_index_bytes: x(E, F + 11),
						material_index_bytes: x(E, F + 12),
						normal_coordinate_bytes: x(E, F + 13),
						uv_coordinate_bytes: x(E, F + 14),
						nvertices: t(E, F + 15),
						ntri_flat: t(E, F + 15 + 4 * 1),
						ntri_smooth: t(E, F + 15 + 4 * 2),
						nquad_flat: t(E, F + 15 + 4 * 3),
						nquad_smooth: t(E, F + 15 + 4 * 4),
						nnormals: t(E, F + 15 + 4 * 5),
						nuvtri: t(E, F + 15 + 4 * 6),
						nuvquad: t(E, F + 15 + 4 * 7),
					};
					return D
				}
				function j(E, F, D) {
				...
				function B(G, F) {
					...
					return E * (1 + D * Math.pow(2, -23)) * Math.pow(2, I)
				}
				function t(H, I) {
					var G = x(H, I),
						F = x(H, I + 1),
				...
				function x(D, E) {
					return D.charCodeAt(E) & 255
				}
				function h(I) {
					var E, D, H, G, F = C.vertex_coordinate_bytes * 3;
					for(E = 0; E < C.nvertices; ++E) {
						D = B(c, I + E * F);
						H = B(c, I + E * F + C.vertex_coordinate_bytes);
						G = B(c, I + E * F + C.vertex_coordinate_bytes * 2);
						g.vertices.push(new THREE.Vertex(new THREE.Vector3(D, H, G)))
					}
					return C.nvertices * F
				}
				function p(K) {
					var G, F, E, J, D, H, I = C.vertex_index_bytes * 3 + C.material_index_bytes;
					for(G = 0; G < C.ntri_flat; ++G) {
						F = t(c, K + G * I);
						E = t(c, K + G * I + C.vertex_index_bytes);
						J = t(c, K + G * I + C.vertex_index_bytes * 2);
						D = y(c, K + G * I + C.vertex_index_bytes * 3);
						H = g.materials[D];
						g.faces.push(new THREE.Face3(F, E, J, null, H))
					}
					return C.ntri_flat * I
				}
				function u(D) {
					var Q, T, S, R, P, J, I, H, K, G, F, E, O, M, L, W, V, U, N = C.vertex_index_bytes * 3 + C.material_index_bytes + C.normal_index_bytes * 3;
					for(Q = 0; Q < C.ntri_smooth; ++Q) {
						T = t(c, D + Q * N);
						S = t(c, D + Q * N + C.vertex_index_bytes);
						R = t(c, D + Q * N + C.vertex_index_bytes * 2);
						P = y(c, D + Q * N + C.vertex_index_bytes * 3);
						J = t(c, D + Q * N + C.vertex_index_bytes * 3 + C.material_index_bytes);
						I = t(c, D + Q * N + C.vertex_index_bytes * 3 + C.material_index_bytes + C.normal_index_bytes);
						H = t(c, D + Q * N + C.vertex_index_bytes * 3 + C.material_index_bytes + C.normal_index_bytes * 2);
						K = g.materials[P];
						G = r[J * 3], F = r[J * 3 + 1], E = r[J * 3 + 2], O = r[I * 3], M = r[I * 3 + 1], L = r[I * 3 + 2], W = r[H * 3], V = r[H * 3 + 1], U = r[H * 3 + 2];
						g.faces.push(new THREE.Face3(T, S, R, [new THREE.Vector3(G, F, E), new THREE.Vector3(O, M, L), new THREE.Vector3(W, V, U)], K))
					}
					return C.ntri_smooth * N
				}
				function m(E) {
					var G, L, K, J, I, F, H, D = C.vertex_index_bytes * 4 + C.material_index_bytes;
					for(G = 0; G < C.nquad_flat; ++G) {
						L = t(c, E + G * D);
						K = t(c, E + G * D + C.vertex_index_bytes);
						J = t(c, E + G * D + C.vertex_index_bytes * 2);
						I = t(c, E + G * D + C.vertex_index_bytes * 3);
						F = y(c, E + G * D + C.vertex_index_bytes * 4);
						H = g.materials[F];
						g.faces.push(new THREE.Face4(L, K, J, I, null, H))
					}
					return C.nquad_flat * D
				}
				function o(D) {
					var U, Y, X, W, V, T, N, M, L, J, O, I, G, E, S, Q, P, ab, aa, Z, K, H, F, R = C.vertex_index_bytes * 4 + C.material_index_bytes + C.normal_index_bytes * 4;
					for(U = 0; U < C.nquad_smooth; ++U) {
						Y = t(c, D + U * R);
						X = t(c, D + U * R + C.vertex_index_bytes);
						W = t(c, D + U * R + C.vertex_index_bytes * 2);
						V = t(c, D + U * R + C.vertex_index_bytes * 3);
						T = y(c, D + U * R + C.vertex_index_bytes * 4);
						N = t(c, D + U * R + C.vertex_index_bytes * 4 + C.material_index_bytes);
						M = t(c, D + U * R + C.vertex_index_bytes * 4 + C.material_index_bytes + C.normal_index_bytes);
						L = t(c, D + U * R + C.vertex_index_bytes * 4 + C.material_index_bytes + C.normal_index_bytes * 2);
						J = t(c, D + U * R + C.vertex_index_bytes * 4 + C.material_index_bytes + C.normal_index_bytes * 3);
						O = g.materials[T];
						I = r[N * 3], G = r[N * 3 + 1], E = r[N * 3 + 2], S = r[M * 3], Q = r[M * 3 + 1], P = r[M * 3 + 2], ab = r[L * 3], aa = r[L * 3 + 1], Z = r[L * 3 + 2];
						K = r[J * 3], H = r[J * 3 + 1], F = r[J * 3 + 2];
						g.faces.push(new THREE.Face4(Y, X, W, V, [new THREE.Vector3(I, G, E), new THREE.Vector3(S, Q, P), new THREE.Vector3(ab, aa, Z), new THREE.Vector3(K, H, F)], O))
					}
					return C.nquad_smooth * R
				}
				function s(G) {
					var I, F, D, M, L, K, J, H, E = C.uv_coordinate_bytes * 6;
					for(I = 0; I < C.nuvtri; I++) {
						F = B(c, G + I * E);
						L = B(c, G + I * E + 4);
						D = B(c, G + I * E + 8);
						K = B(c, G + I * E + 12);
						...
						J = B(c, G + I * E + 20);
						H = [];
						H.push(new THREE.UV(F, L));
						H.push(new THREE.UV(D, K));
						H.push(new THREE.UV(M, J));
						g.uvs.push(H)
					}
					return C.nuvtri * E
				}
				function w(G) {
					var I, F, D, O, N, M, L, K, J, H, E = C.uv_coordinate_bytes * 8;
					for(I = 0; I < C.nuvquad; I++) {
						F = B(c, G + I * E);
						M = B(c, G + I * E + 4);
						D = B(c, G + I * E + 8);
						L = B(c, G + I * E + 12);
						...
						J = B(c, G + I * E + 28);
						H = [];
						H.push(new THREE.UV(F, M));
						H.push(new THREE.UV(D, L));
						H.push(new THREE.UV(O, K));
						H.push(new THREE.UV(N, J));
						g.uvs.push(H)
					}
					return C.nuvquad * E
				}
				function l(I) {
					var E, D, H, G, F = C.normal_coordinate_bytes * 3;
					for(E = 0; E < C.nnormals; ++E) {
						D = q(c, I + E * F);
						H = q(c, I + E * F + C.normal_coordinate_bytes);
						G = q(c, I + E * F + C.normal_coordinate_bytes * 2);
						r.push(D / 127, H / 127, G / 127)
					}
					return C.nnormals * F
				}
				function n() {
					...
					for(var D = 0; D < a.length; ++D) {
						g.materials[D] = [A(a[D], k)]
					}
				}
				function f(E) {
					var D = Math.log(E) / Math.LN2;
					return Math.floor(D) == D
				}
				function i(E) {
					var D = Math.log(E) / Math.LN2;
					return Math.pow(2, Math.round(D))
				}
				function A(D) {
					var F, G, H, E;
					if(D.map_diffuse && k) {
						G = document.createElement("canvas");
						F = new THREE.MeshBitmapMaterial(G);
						H = new Image();
						H.onload = function() {
							if(!f(this.width) || !f(this.height)) {
								var I = i(this.width),
									J = i(this.height);
								F.bitmap.width = I;
								F.bitmap.height = J;
								F.bitmap.getContext("2d").drawImage(this, 0, 0, I, J)
							} else {
								F.bitmap = this
							}
							F.loaded = 1
						};
						H.src = k + "/" + D.map_diffuse
					} else {
						if(D.col_diffuse) {
							E = (D.col_diffuse[0] * 255 << 16) + (D.col_diffuse[1] * 255 << 8) + D.col_diffuse[2] * 255;
							F = new THREE.MeshColorFillMaterial(E, D.transparency)
						} else {
							if(D.a_dbg_color) {
								F = new THREE.MeshColorFillMaterial(D.a_dbg_color)
							} else {
								F = new THREE.MeshColorFillMaterial(15658734)
							}
						}
					}
					return F
					...
					if(t.map_diffuse && f) {
						...
						if(t.col_diffuse) {
							...
							if(t.a_dbg_color) {
								...
								v = new THREE.MeshColorFillMaterial(15658734)
							}
						}
					}
					return v
...
THREE.SVGRenderer = function() {
	...
	this.setSize = function(D, C) {
		...
		p = b / 2;
		s = o / 2;
		t.setAttribute("viewBox", (-p) + " " + (-s) + " " + b + " " + o);
		t.setAttribute("width", b);
	...
	function j(C, G, D, E, F) {
		...
		if(E instanceof THREE.ParticleCircleMaterial) {
			...
			B.setAttribute("style", "fill: " + k.__styleString)
		}
		t.appendChild(B)
	}
	function h(E, D, C, K, J, I, F, H, G) {
		...
		if(H instanceof THREE.MeshColorFillMaterial) {
			...
			if(H instanceof THREE.MeshColorStrokeMaterial) {
				...
				B.setAttribute("style", "fill: none; stroke: " + k.__styleString + "; stroke-width: " + H.lineWidth + "; stroke-linecap: round; stroke-linejoin: round")
			}
		}
		t.appendChild(B)
	}
	function e(G, E, C, M, L, K, F, D, H, J, I) {
		...
		if(J instanceof THREE.MeshColorFillMaterial) {
			...
			if(J instanceof THREE.MeshColorStrokeMaterial) {
				...
				B.setAttribute("style", "fill: none; stroke: " + k.__styleString + "; stroke-width: " + J.lineWidth + "; stroke-linecap: round; stroke-linejoin: round")
			}
		}
		t.appendChild(B)
	}
	...
	function u(C) {
		if(l[C] == null) {
			l[C] = document.createElementNS("http://www.w3.org/2000/svg", "circle");
			if(A == 0) {
				l[C].setAttribute("shape-rendering", "crispEdges")
			}
			return l[C]
		}
		return l[C]
...
THREE.WebGLRenderer = function(q) {
	var h = document.createElement("canvas"),
		f, n, k = new THREE.Matrix4(),
		d, g = 0,
		c = 1,
		s = 2,
		o = 3,
	...
	this.autoClear = true;
	a();
	i(l.directional, l.point);

	function b(x, y) {
		if(x) {
			var u, w, t, v = pointLights = maxDirLights = maxPointLights = 0;
			for(u = 0, w = x.lights.length; u < w; u++) {
				t = x.lights[u];
				if(t instanceof THREE.DirectionalLight) {
					v++
				}
				if(t instanceof THREE.PointLight) {
					pointLights++
				}
			}
			if((pointLights + v) <= y) {
				...
				maxDirLights = Math.ceil(y * v / (pointLights + v));
				maxPointLights = y - maxDirLights
			}
			return {
	...
	this.setSize = function(u, t) {
		h.width = u;
		h.height = t;
		f.viewport(0, 0, h.width, h.height)
	};
	this.clear = function() {
		f.clear(f.COLOR_BUFFER_BIT | f.DEPTH_BUFFER_BIT)
	};
	this.setupLights = function(z) {
		var w, D, x, u, A, E, v = [],
			B = [],
			C = [],
			t = [],
			y = [];
		f.uniform1i(n.enableLighting, z.lights.length);
		for(w = 0, D = z.lights.length; w < D; w++) {
			x = z.lights[w];
		...
		u = A = E = 0;
		for(w = 0, D = v.length; w < D; w++) {
			u += v[w].color.r;
			A += v[w].color.g;
			E += v[w].color.b
		}
		f.uniform3f(n.ambientLightColor, u, A, E);
		t = [];
		y = [];
		for(w = 0, D = C.length; w < D; w++) {
			x = C[w];
			t.push(x.color.r * x.intensity);
			t.push(x.color.g * x.intensity);
			t.push(x.color.b * x.intensity);
			y.push(x.position.x);
			y.push(x.position.y);
			y.push(x.position.z)
		}
		if(C.length) {
			f.uniform1i(n.directionalLightNumber, C.length);
			f.uniform3fv(n.directionalLightDirection, y);
			f.uniform3fv(n.directionalLightColor, t)
		}
		t = [];
		y = [];
		for(w = 0, D = B.length; w < D; w++) {
			x = B[w];
			t.push(x.color.r * x.intensity);
			t.push(x.color.g * x.intensity);
			t.push(x.color.b * x.intensity);
		...
		if(B.length) {
			f.uniform1i(n.pointLightNumber, B.length);
			f.uniform3fv(n.pointLightPosition, y);
			f.uniform3fv(n.pointLightColor, t)
		}
	};
	this.createBuffers = function(M, K) {
		var I, A, C, z, H, L, y, w, v, u, t, x = M.materialFaceGroup[K],
			E = [],
			G = [],
			D = [],
			J = [],
			F = [],
			B = 0;
		for(I = 0, A = x.faces.length; I < A; I++) {
			C = x.faces[I];
			z = M.geometry.faces[C];
			H = z.vertexNormals;
			L = z.normal;
			y = M.geometry.uvs[C];
			if(z instanceof THREE.Face3) {
				w = M.geometry.vertices[z.a].position;
				v = M.geometry.vertices[z.b].position;
				u = M.geometry.vertices[z.c].position;
				D.push(w.x, w.y, w.z);
				D.push(v.x, v.y, v.z);
				D.push(u.x, u.y, u.z);
				if(H.length == 3) {
					J.push(H[0].x, H[0].y, H[0].z);
					J.push(H[1].x, H[1].y, H[1].z);
					J.push(H[2].x, H[2].y, H[2].z)
				} else {
					J.push(L.x, L.y, L.z);
					J.push(L.x, L.y, L.z);
					J.push(L.x, L.y, L.z)
				...
				if(z instanceof THREE.Face4) {
					w = M.geometry.vertices[z.a].position;
					v = M.geometry.vertices[z.b].position;
					u = M.geometry.vertices[z.c].position;
					t = M.geometry.vertices[z.d].position;
					D.push(w.x, w.y, w.z);
					D.push(v.x, v.y, v.z);
					D.push(u.x, u.y, u.z);
					D.push(t.x, t.y, t.z);
		...
		x.__webGLVertexBuffer = f.createBuffer();
		f.bindBuffer(f.ARRAY_BUFFER, x.__webGLVertexBuffer);
		f.bufferData(f.ARRAY_BUFFER, new Float32Array(D), f.STATIC_DRAW);
		x.__webGLNormalBuffer = f.createBuffer();
		f.bindBuffer(f.ARRAY_BUFFER, x.__webGLNormalBuffer);
		f.bufferData(f.ARRAY_BUFFER, new Float32Array(J), f.STATIC_DRAW);
		x.__webGLUVBuffer = f.createBuffer();
		f.bindBuffer(f.ARRAY_BUFFER, x.__webGLUVBuffer);
		f.bufferData(f.ARRAY_BUFFER, new Float32Array(F), f.STATIC_DRAW);
		x.__webGLFaceBuffer = f.createBuffer();
		f.bindBuffer(f.ELEMENT_ARRAY_BUFFER, x.__webGLFaceBuffer);
		f.bufferData(f.ELEMENT_ARRAY_BUFFER, new Uint16Array(E), f.STATIC_DRAW);
		x.__webGLLineBuffer = f.createBuffer();
		f.bindBuffer(f.ELEMENT_ARRAY_BUFFER, x.__webGLLineBuffer);
		f.bufferData(f.ELEMENT_ARRAY_BUFFER, new Uint16Array(G), f.STATIC_DRAW);
		x.__webGLFaceCount = E.length;
		x.__webGLLineCount = G.length
	};
	this.renderBuffer = function(u, t) {
		if(u instanceof THREE.MeshPhongMaterial) {
			mAmbient = u.ambient;
			mDiffuse = u.diffuse;
			mSpecular = u.specular;
			f.uniform4f(n.mAmbient, mAmbient.r, mAmbient.g, mAmbient.b, u.opacity);
			f.uniform4f(n.mDiffuse, mDiffuse.r, mDiffuse.g, mDiffuse.b, u.opacity);
			f.uniform4f(n.mSpecular, mSpecular.r, mSpecular.g, mSpecular.b, u.opacity);
			f.uniform1f(n.mShininess, u.shininess);
			f.uniform1i(n.material, o)
		} else {
			if(u instanceof THREE.MeshColorFillMaterial) {
				color = u.color;
				f.uniform4f(n.mColor, color.r * color.a, color.g * color.a, color.b * color.a, color.a);
				f.uniform1i(n.material, g)
			} else {
				if(u instanceof THREE.MeshColorStrokeMaterial) {
					lineWidth = u.lineWidth;
					color = u.color;
					f.uniform4f(n.mColor, color.r * color.a, color.g * color.a, color.b * color.a, color.a);
					f.uniform1i(n.material, c)
				} else {
					if(u instanceof THREE.MeshBitmapMaterial) {
						if(!u.__webGLTexture && u.loaded) {
							u.__webGLTexture = f.createTexture();
							f.bindTexture(f.TEXTURE_2D, u.__webGLTexture);
							f.texImage2D(f.TEXTURE_2D, 0, f.RGBA, f.RGBA, f.UNSIGNED_BYTE, u.bitmap);
						...
						f.bindTexture(f.TEXTURE_2D, u.__webGLTexture);
						f.uniform1i(n.tDiffuse, 0);
						f.uniform1i(n.material, s)
					}
				}
			}
		}
		f.bindBuffer(f.ARRAY_BUFFER, t.__webGLVertexBuffer);
		f.vertexAttribPointer(n.position, 3, f.FLOAT, false, 0, 0);
		f.bindBuffer(f.ARRAY_BUFFER, t.__webGLNormalBuffer);
		f.vertexAttribPointer(n.normal, 3, f.FLOAT, false, 0, 0);
		if(u instanceof THREE.MeshBitmapMaterial) {
			f.bindBuffer(f.ARRAY_BUFFER, t.__webGLUVBuffer);
			f.enableVertexAttribArray(n.uv);
			f.vertexAttribPointer(n.uv, 2, f.FLOAT, false, 0, 0)
		} else {
			f.disableVertexAttribArray(n.uv)
		}
		if(u instanceof THREE.MeshBitmapMaterial || u instanceof THREE.MeshColorFillMaterial || u instanceof THREE.MeshPhongMaterial) {
			f.bindBuffer(f.ELEMENT_ARRAY_BUFFER, t.__webGLFaceBuffer);
			f.drawElements(f.TRIANGLES, t.__webGLFaceCount, f.UNSIGNED_SHORT, 0)
		} else {
			if(u instanceof THREE.MeshColorStrokeMaterial) {
				f.lineWidth(lineWidth);
				f.bindBuffer(f.ELEMENT_ARRAY_BUFFER, t.__webGLLineBuffer);
				f.drawElements(f.LINES, t.__webGLLineCount, f.UNSIGNED_SHORT, 0)
			}
		}
	};
	this.renderMesh = function(v, y) {
		var x, u, t, w, B, z, A, C;
		for(B in v.materialFaceGroup) {
			C = v.materialFaceGroup[B];
			if(!C.__webGLVertexBuffer) {
				this.createBuffers(v, B)
			}
			for(t = 0, w = v.material.length; t < w; t++) {
				A = v.material[t];
				if(A instanceof THREE.MeshFaceMaterial) {
					for(x = 0, u = C.material.length; x < u; x++) {
						z = C.material[x];
						this.renderBuffer(z, C)
					}
				} else {
					z = A;
					this.renderBuffer(z, C)
	...
	this.setupMatrices = function(t, u) {
		t.autoUpdateMatrix && t.updateMatrix();
		k.multiply(u.matrix, t.matrix);
		n.viewMatrixArray = new Float32Array(u.matrix.flatten());
		n.modelViewMatrixArray = new Float32Array(k.flatten());
		n.projectionMatrixArray = new Float32Array(u.projectionMatrix.flatten());
		d = THREE.Matrix4.makeInvert3x3(k).transpose();
		n.normalMatrixArray = new Float32Array(d.m);
		f.uniformMatrix4fv(n.viewMatrix, false, n.viewMatrixArray);
		f.uniformMatrix4fv(n.modelViewMatrix, false, n.modelViewMatrixArray);
		f.uniformMatrix4fv(n.projectionMatrix, false, n.projectionMatrixArray);
		f.uniformMatrix3fv(n.normalMatrix, false, n.normalMatrixArray);
		f.uniformMatrix4fv(n.objMatrix, false, new Float32Array(t.matrix.flatten()))
	};
	this.render = function(w, v) {
		var x, u, t;
		if(this.autoClear) {
			this.clear()
		}
		v.autoUpdateMatrix && v.updateMatrix();
		f.uniform3f(n.cameraPosition, v.position.x, v.position.y, v.position.z);
		this.setupLights(w);
		for(x = 0, u = w.objects.length; x < u; x++) {
			t = w.objects[x];
			this.setupMatrices(t, v);
			if(t instanceof THREE.Mesh) {
				this.renderMesh(t, v)
			} else {
				if(t instanceof THREE.Line) {} else {
					if(t instanceof THREE.Particle) {}
				}
			}
		}
	};
	this.setFaceCulling = function(u, t) {
		if(u) {
			if(!t || t == "ccw") {
				f.frontFace(f.CCW)
			} else {
	...
	function a() {
		try {
			f = h.getContext("experimental-webgl", {
				antialias: true
			})
		} catch(t) {}
		if(!f) {
	...
	function r(u, t) {
		...
		f.shaderSource(v, t);
		f.compileShader(v);
		if(!f.getShaderParameter(v, f.COMPILE_STATUS)) {
			alert(f.getShaderInfoLog(v));
			return null
		}
		return v
	}
	function e() {
		var t = {
			MAX_VARYING_VECTORS: f.getParameter(f.MAX_VARYING_VECTORS),
			MAX_VERTEX_ATTRIBS: f.getParameter(f.MAX_VERTEX_ATTRIBS),
			MAX_TEXTURE_IMAGE_UNITS: f.getParameter(f.MAX_TEXTURE_IMAGE_UNITS),
			MAX_VERTEX_TEXTURE_IMAGE_UNITS: f.getParameter(f.MAX_VERTEX_TEXTURE_IMAGE_UNITS),
			MAX_COMBINED_TEXTURE_IMAGE_UNITS: f.getParameter(f.MAX_COMBINED_TEXTURE_IMAGE_UNITS),
...
var SCREEN_WIDTH = window.innerWidth - 22;
